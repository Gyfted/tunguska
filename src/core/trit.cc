 ...
 * C++17 compatibility and valid ternary permutation/shift results.
...
tval_t trit::trit_strategy_tuf_calculate::op_tsh(tval_t a, tval_t b) const {
	return std::clamp(a + b, -1, 1);
}
tval_t trit::trit_strategy_tuf_calculate::op_rol(tval_t a, tval_t b) const {
	// C++ remainder can be negative; normalize before mapping back to a trit.
	return (a + b + 4) % 3 - 1;
}
