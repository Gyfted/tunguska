...
class expression {
	public:
		...
		virtual void autoeval() const {
			switch(typ()->size()) {
				...
				case 2:
					eval12(); break;
				default:
					throw std::runtime_error("Evaluating expression of bad size");
			}
		}
		...
		virtual ~expression() { allocated_expressions.remove(this); }
...
class constant : public expression {
	public:
		...
		virtual void eval12() const;
		virtual const type* typ() const { 
			if(value < -364 || value > 364) return new t_i12();
...
public:
	...
	virtual const type* typ() const {
		...
		} catch(const std::runtime_error& e) { // Function not yet initialized
	...
	public:
		assigner(expression* t, expression* a) : univalent(a), target(t) {
			if(dynamic_cast<lvalue*>(t) == NULL) 
				throw runtime_error("Assigning non-lvalue");
		}
...
class pre_increment : public expression {
	public:
		pre_increment(expression* e) {
			...
			if(!lval) throw runtime_error("Bad lvalue");
		}
		~pre_increment() {}
