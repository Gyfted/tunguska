...
symbol_table_entry* function::sym_ref(const char* name) {
	...
	if(vars.is_var(name)) {
		...
		snprintf(namebuf, strlen(name)+2, ".%s", name);
		return new symbol_table_entry(namebuf, vars.get_offset(name),  vars.get_variable(name));
	}
	...
	throw invalid_argument("function::sym_ref");
}
...
void function::do_return(expression* e) {
	if(!e) {
		...
		if(get_type()->size() != 0) 
			throw std::runtime_error("Returning with no argument from"
	...
	switch(get_type()->size()) {
		case 0:
			throw std::runtime_error("Returning non-void from void function");
		case 1:
			e->eval6();
		...
		case 2:	
			...
			break;
		default:
			throw std::runtime_error("Returning impossible type");
	} 
...
void function::add_scope(sllist* variables) {
	...
	while(node) {
		...
		char* namebuf = strdup((std::string(v->get_name()) + ".s" + std::to_string(scope_level)).c_str());
		if(v->is_static()) 
	...
	if (newsize > 364) throw runtime_error("Function frame exceeds the 364-tryte variable stack");
	if(newsize - oldsize) {
...
void function::bail_scopes(int levels) {
	...
	int total_offset = 0;
	while(levels--) {
		if(i == scopes.end()) throw runtime_error("Bailing too many scopes\n");
		scope* s = (*i);
...
void function::init_definition(const function_prototype& f, const char* name, const type* rettype, sllist* args, sllist* locals) {
	...
	if(workbench) {
		throw runtime_error("Function definition within function.");
	}
...
int function::init_varstack(sllist* locals) {
	...
	if (to > 364) throw runtime_error("Function frame exceeds the 364-tryte variable stack");
	if(to) {
		if(to == 1) 
			compiler::instance()->printf("\t\tDEC\t__VSS\n");
		else { 
...
void function::finish_definition() {
	...
	for(i = variables.begin(); i != variables.end(); i++) {
		...
		snprintf(namep, 2+strlen(v->get_name()), ".%s", v->get_name());
