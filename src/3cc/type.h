...
// Types are shared throughout the historical compiler graph. A complete
// ownership redesign remains future work; compilation uses a separate process.

class type {
	public:
		class not_a_reference : public std::runtime_error {
		public: not_a_reference() : std::runtime_error("Dereferencing a non-pointer type") {}
		};
		type() {}
		virtual ~type() {};
...
public:
	t_array(type* base, int elements) {
		if (elements <= 0 || base->size() <= 0 || elements > 531441 / base->size())
			throw std::runtime_error("Array size must be positive and fit guest memory");
		this->base = base;
