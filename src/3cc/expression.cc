...
expression* expression::dearray(expression* a) {
	if (!a) throw runtime_error("Missing expression");
	const t_array* t = dynamic_cast<const t_array*>(a->typ());
	if(!t) return a;

	word_caster* wc = dynamic_cast<word_caster*>(a);
	if(wc) a = wc->get_base();
	if (!a) throw runtime_error("Missing array expression");
	...
	throw runtime_error("Dearray failed");
}
...
lvalue* array_index(expression* a, expression* b) {
	...
	if(b->is_const()) {
		...
		if(!lva) throw runtime_error("Bad array");
...
lvalue* struct_ptr_field(expression* e, const char* field) {
	if(!e->typ()->can_deref())
		throw std::runtime_error("Bad field dereference");

	const t_struct* source_type = dynamic_cast<const t_struct*>(e->typ()->dereference());
	if (!source_type) throw runtime_error("Field access requires a struct pointer");
...
function_call::function_call(const char* name, const function_prototype& f, sllist* arg) : fun(f) {
	...
	if(args.size() != fun.get_args().size()) {
		throw runtime_error("Call to " + std::string(name) + " with mismatching number of arguments");
	} 
}
...
const char* symbol::get_name() const {
	...
	char* ename;
	if(offset) {
		ename = strdup((std::string(buf) + "+" + std::to_string(offset)).c_str());
	} else {
		ename = strdup(buf);
	}
	delete info;
...
lvalue* symbol::struct_field(const char* field) {
	...
	if(!struct_type) throw runtime_error("Not a struct.");
...
void symbol::eval6() const {
	switch(typ()->size()) {
		case 0:
			throw runtime_error("Evaluating void " + std::string(get_name()));
		case 1:
	...
	if(da) {
		...
		switch(typ()->size()) {
			...
			case 2:
				...
				break;
			default: 
				throw runtime_error("Assigning non-plain-data");
		}
	} else {
		if(typ()->size() == 1) {
			e->eval6();
		...
		} else throw runtime_error("Assigning non-plain-data");
	}
}
