/* Modified 2026-09-24: checked input, staged output and C++17 diagnostics. */
...
class compiler {
	public:
		...
		/* File output */
		int printf(const char* arg, ...) __attribute__((format(printf, 2, 3)));
		...
		void set_effective_location(const char* file, int lineno) {
			if(file) effective_filename = file;
			line = lineno;
		}

		const char* get_effective_file() { return effective_filename.c_str(); }
	private:
