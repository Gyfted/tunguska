/* Modified 2026-09-24: encode guest strings as checked numeric trytes. */
...
extern int char_constant(char);
extern int escape_char(char);
string_node::string_node(const char* str) {
    while (*str) {
        if (*str == '\\') {
            ++str;
            if (!*str) throw std::runtime_error("Incomplete string escape");
            data.push_back(escape_char(*str++));
        } else data.push_back(char_constant(*str++));
    }
}
void string_node::define() const {
    compiler::instance()->printf("\t@DT\t");
    for (int value : data) compiler::instance()->printf("%d,", value);
    compiler::instance()->printf("0\n");
}
int string_node::size() const { return static_cast<int>(data.size()) + 1; }
