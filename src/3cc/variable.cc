...
void composite_initializer::initialize(expression *sym) {
	...
	if(ts) { /* Initialize a struct */
		...
		while(node) {
			if(ri == fields.end()) throw runtime_error("Mismatching struct size/initializer field count");
		...
		for(; node; element++,node=node->next) {
			if(element >= ta->dimension()) throw runtime_error("Array initializer too big");
	...
	} else throw std::runtime_error("What are you trying to initialize?!");
}
...
void composite_initializer::define() {
	...
	if(ts) { /* Initialize a struct */
		...
		if(node) throw runtime_error("Array initializer too big");
		if(element < ta->dimension()) compiler::instance()->printf("@REST %d\n", (ta->dimension() - element) * ta->dereference()->size());
	} else {
		typ->id();
		throw std::runtime_error("What are you trying to define?!");
	}
...
void variable_set::add_var(variable* v) {
	...
	if(is_var(v->get_name())) {
		...
		throw runtime_error("Variable " + std::string(v->get_name()) + " added twice!");
	}
...
variable* variable_set::get_variable(const char* name) {
	...
	throw invalid_argument("Failed to look up variable " + std::string(name));
}
