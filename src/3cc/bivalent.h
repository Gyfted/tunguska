	...
	public:
		bivalent(expression* a, expression* b) { 
			try {
				...
				/* Exception values are managed automatically. */
				this->a = a;
			}
			try {
				this->b = dearray(b); 
...
class adder : public bivalent {
	public:
		...
		virtual bool is_const() { return a->is_const() && b->is_const(); }
		virtual int const_val() { return checked_word(int64_t(a->const_val()) + b->const_val()); }
};
...
class divider : public bivalent {
	public:
		...
		virtual int const_val() { return checked_quotient(a->const_val(), b->const_val()); }
...
class modulo : public bivalent {
	public:
		...
		virtual bool is_const() { return a->is_const() && b->is_const(); }
					// TODO: Make sure b != NULL
		virtual int const_val() { return checked_quotient(a->const_val(), b->const_val(), true); }
...
class logical_ander : public ander {
	public:
		logical_ander(expression* a, expression* b) : ander(a,b) {
			if(!this->a->is_logical()) this->a = new sign(this->a);
			if(!this->b->is_logical()) this->b = new sign(this->b);
		}
