...
def prepare(args):
    ...
    with (folder / "build-and-tests.log").open("w") as log:
        subprocess.run(["make", "-j4", "ARCHS=" + " ".join(args.archs), "all", "test", "sanitize", "security-check", "compiler-check", "compiler-sanitize", "vision-check", "vision-sanitize", "explorer-check", "explorer-sanitize", "weight-check", "weight-sanitize", "weight-gpu-validation", "verify-app", "release-check"],
