...
def prepare(args):
    ...
    with (folder / "build-and-tests.log").open("w") as log:
        subprocess.run(["make", "-j4", "ARCHS=" + " ".join(args.archs), "all", "test", "sanitize", "security-check", "compiler-check", "compiler-sanitize", "vision-check", "vision-sanitize", "verify-app", "release-check"],
